...
- Cargo.toml のバージョン変更をトリガーにリリースプロセスを自動化する
...
main ブランチへの push で `Cargo.toml` が変更された場合に起動する。タグの手動作成は不要。
...
│  - 同名タグが既存か確認                        │
│  - バージョン変更なしなら後続ジョブをスキップ     │
└──────────────────┬──────────────────────────┘
                   │ needs: check-version (changed == true)
┌──────────────────▼──────────────────────────┐
...
┌──────────────────▼──────────────────────────┐
│  update-homebrew                            │
│  - repository-dispatch で homebrew-tap へ通知 │
│  - homebrew-tap 側で formula を更新           │
└─────────────────────────────────────────────┘
```
...
2. コミット＆プッシュ（main ブランチへマージ）
3. release workflow が自動的に以下を実行:
   - バージョン変更の検知（既存タグとの比較）
   - マルチプラットフォームバイナリのビルド
   - タグの自動作成・push
   - GitHub Release の作成（リリースノート自動生成）
   - crates.io への publish
   - Homebrew formula の更新（repository-dispatch 経由）
