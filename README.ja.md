...
[git worktree](https://git-scm.com/docs/git-worktree) と組み合わせて複数の AI エージェントを並列実行するワークフローが注目されています。`ghostty-pane-splitter` なら tmux 不要でマルチエージェントレイアウトを瞬時にセットアップできます。
...
> **重要**: キーバインドを追加・変更した後は、Ghostty を再起動してください。更新した設定で Ghostty を起動し直すと正しく動作するようになります。

> **Tips** (Ghostty 1.3.0+): デフォルトでは、新しいスプリットは現在のペインの作業ディレクトリを引き継ぎます。`split-inherit-working-directory` 設定でこの動作を変更できます。新しいスプリットを常にホームディレクトリで開始したい場合は `false` に設定してください。
> ```
...
| 種類 | コマンド | デモ |
|------|---------|------|
| 数値指定 | `ghostty-pane-splitter 9` | ![number](images/demo-number.gif) |
| グリッド指定 | `ghostty-pane-splitter 2x2` | ![grid](images/demo-grid.gif) |
| カスタムレイアウト | `ghostty-pane-splitter 1,4` | ![custom](images/demo-custom.gif) |
